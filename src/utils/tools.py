...
def frozen_without_last_module(model: torch.nn.Module) -> None:
    """Freeze all parameters except the last module.
...
def frozen_last_module(model: torch.nn.Module) -> None:
    """Freeze all parameters except the last module.
...
def unfronzen(model: torch.nn.Module) -> None:
...
def prt_trainable_param(model: torch.nn.Module) -> None:
    """Print the trainable status of each parameter in the model.
...
def transfer_weights_without_last_module(model1: torch.nn.Module, model2: torch.nn.Module) -> None:
    """Transfer weights from `model1` to `model2` without the last module.

    Args:
        model1 (torch.nn.Module): The source model.
        model2 (torch.nn.Module): The target model.
    """
    # This function assumes that model1 and model2 have exactly the same architecture

    # Get the sub-modules of the model
    children1 = list(model1.children())
    children2 = list(model2.children())

    # Copy all weights except the last layer
    for i in range(len(children1)):
        # If it is not the last module, copy all weights
        if i != len(children1) - 1:
            children2[i].load_state_dict(children1[i].state_dict())
        
        # If it is the last module, don't copy the weights
