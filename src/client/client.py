...
class LoopClient:
    def __init__(
        ...
        if self.args.optimizer == "sgd":
            ...
            frozen_without_last_module(self.model)
            ...
            frozen_last_module(self.model)
            self.optimizer_backbone = torch.optim.SGD(
                params=filter(lambda p: p.requires_grad, self.model.parameters()),
                lr=self.args.local_lr,
                momentum=self.args.momentum,
                weight_decay=self.args.weight_decay,
            )
            unfronzen(self.model)
    ...
    def fit_round(self):
        """The function for specifying operations in local training phase for each round"""
    ...
    def training_step(self, step):
        """The function for specifying operations in local training phase for each step"""
        if step == 0:
            self.model.train()
            for _ in range(self.local_epoch):
                for x, y in self.trainloader:
